...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'search_products') {
        ...
        $sql = "SELECT id, name, selling_price FROM products WHERE name LIKE '%$query%' OR sku LIKE '%$query%' LIMIT 10";
        $result = mysqli_query($conn, $sql);
        $products = mysqli_fetch_all($result, MYSQLI_ASSOC);
        echo json_encode($products);
    }

    if (isset($_POST['action']) && $_POST['action'] === 'process_invoice_sale') {
        $items = $_POST['items'];
        ...
        $sql_sale = "INSERT INTO sales (user_id, sale_date, total_amount, tax_amount, grand_total, payment_method, status, receipt_no)
                     VALUES ('$user_id', '$sale_date', '$subtotal', '$tax_amount', '$grand_total', 'Cash', 'Completed', '$receipt_no')";
